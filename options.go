...
type config struct {
	...
	decompressFn           func(c *touka.Context)
	decompressOnly         bool
	customShouldCompressFn func(c *touka.Context) bool
}
...
// Parameters:
//   - decompressFn: func(c *touka.Context) - A function to handle decompression of incoming requests.
func WithDecompressFn(decompressFn func(c *touka.Context)) Option {
	return optionFunc(func(o *config) {
...
// Parameters:
//   - fn: func(c *touka.Context) bool - A function to determine if a request should be compressed.
...
// Example:
//
//	router.Use(gzip.Gzip(gzip.DefaultCompression, gzip.WithCustomShouldCompressFn(func(c *touka.Context) bool {
//		return c.Request.URL.Path != "/no-compress"
//	})))
func WithCustomShouldCompressFn(fn func(c *touka.Context) bool) Option {
	return optionFunc(func(o *config) {
...
// DefaultDecompressHandle is a middleware function for the touka framework that
...
//
// Parameters:
//   - c: *touka.Context - The touka context for the current request.
func DefaultDecompressHandle(c *touka.Context) {
	if c.Request.Body == nil {
		return
	}

	contentEncodingField := strings.Split(strings.ToLower(c.GetReqHeader("Content-Encoding")), ",")
	...
	for i := 0; i < len(contentEncodingField); i++ {
		...
		if trimmedValue != "gzip" {
			...
			// An oritouka server MAY respond with a status code of 415 (Unsupported
